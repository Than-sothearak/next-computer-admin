...
const ChangPasswordForm = ({ userId, userData}) => {
  ...
  return (
    <form action={action} className={`text-xs w-full py-4 gap-2 flex flex-col justify-start items-center`}>
  <div>
  <button
  ...
  onKeyDown={(e) => e.key === " " && e.preventDefault()} 
          ...
          <button className={`text-secondarytext py-3 rounded-xl bg-blue-600 hover:bg-blue-700 font-bold ${
                  isPending ? "opacity-70 cursor-not-allowed" : ""}`}><p>Update</p></button>
        </div>
        </div>
