  ...
  return (
    <>
      <div className="w-full p-4 bg-primary mt-4 rounded-lg">
        <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center  gap-2 text-primary z-50 sm:hidden p-2  bg-red-500 fixed w-22 top-3 transform transition-transform duration-700 ease-in-out ${isOpen ? 'left-0 -translate-x-0 rounded-r-xl' : 'right-0 rounded-l-xl translate-x-0'}`}>
          <MdMenuOpen />
          Order list
        </button>
