...
const OrderList = ({
  ...
  const handleChange = (e) => {
    setInCustomerID(e.target.value);
    setInCustomerPhone(e.target.value)
  }
  ...
  // You can either save it to a database, local storage, or send it to an API
  // For this example, we will log it to the console
              ...
              <td className="p-2 font-bold">
                <div className="p-2 flex max-sm:flex-wrap justify-center items-center gap-2 border rounded-md">
                  ...
                  disabled={unavailableStock}
                    ...
                    type="button"
                    className={`p-2 bg-secondary rounded-md w-full ${unavailableStock ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    +
                  </button>
                </div>
