...
export const Navbar = () => {
  ...
  return (
    <div className="bg-slate-800 py-6 px-4 rounded-lg w-full flex justify-between items-center">
      <div>
        <h2 className="font-bold">{pathName.split("/").pop()}</h2>
      </div>
      <div className="flex gap-4 items-center">
        <div className="bg-slate-600 rounded-md w-40 flex relative items-center justify-start pl-2 gap-2 py-2">
          <FaSearch />
          <input
            type="text"
            placeholder="Search...."
            className="w-28 text-sm border-none outline-none ring-0 focus:ring-0 focus:outline-none bg-transparent absolute top-0 bottom-0 left-7 overflow-hidden"
          />
        </div>
