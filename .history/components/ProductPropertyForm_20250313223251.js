...
export default function ProductPropertyForm({ categoryProperties, }) {
  ...
  useEffect(() => {
    if (categoryProperties?.length > 0) {
      setProperties((prevProperties) => {
        const newProperties = categoryProperties.map((item) => ({
          part: item.part,
          value: "", // Default value (can be updated by user)
        }));
  
        // Avoid duplicates: Check if the part already exists
        const mergedProperties = [...prevProperties];
        newProperties.forEach((newProp) => {
          const exists = mergedProperties.some((prop) => prop.part === newProp.part);
          if (!exists) {
    ...
    <div className="bg-slate-800 w-full rounded-lg border p-4 border-slate-600">
      ...
      <div className="flex gap-4 flex-col mt-4 ">
        {properties?.map((item, index) => (
          <div className="flex flex-col gap-2 border border-slate-500 p-2 rounded-md" key={index}>
