  ...
  return (
    <>
      <div className="w-full p-4 bg-primary mt-4 rounded-lg">
        <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center  gap-2 text-primary z-50 sm:hidden p-2  bg-red-500 fixed w-22 top-1 ${isOpen ? ' rounded-r-xl left-0' : 'right-0 rounded-l-xl'}`}>
          <MdMenuOpen />
          Order list
        </button>
        ...
        <div className="grid sm:grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
      ...
      <div className={`bg-primary mt-4 p-4
         left-0 z-40 transform rounded-lg top-4  max-sm:block transition-transform w-1/2  duration-700 ease-in-out
        ${
        ...
        <div className="flex flex-col gap-2 mt-4 border p-4 rounded-md">
            ...
            <div className="flex gap-2 items-center text-sm">
                ...
                ></input>
                <button onClick={handleGenerate}className="bg-blue-500 p-1 w-full rounded-md text-primary">Auto generate ID</button>
            </div>
        </div>
      </div>
    </>
