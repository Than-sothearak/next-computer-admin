...
export default function ProductForm({
  ...
  const [currentProperties, setCurrentProperties] = useState(formData.properties);
  ...
  const handleCategoryChange = (e) => {
    ...
    const newProperties = categories.find((category) => category._id === categoryId)?.properties || [];
  
    setCurrentProperties((prevProperties) => {
      // Create a Set to store unique parts
      const uniqueParts = new Set(prevProperties.map((prop) => prop.part));
  
      // Filter out new properties that already exist
      const filteredNewProperties = newProperties.filter((prop) => !uniqueParts.has(prop.part));
            ...
            <div className=" bg-slate-800 p-4 w-full flex  flex-col  gap-4 rounded-lg">
              ...
              >
                {formData?.parentCategory && (
                  <option defaultValue={formData?.parentCategory._id}>
