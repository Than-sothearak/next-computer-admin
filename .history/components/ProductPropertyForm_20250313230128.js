...
export default function ProductPropertyForm({ categoryProperties, productProperties, formData }) {
 ...
 console.log(formData.parentCategory)
...
{formData.properties?.map((items, index) => (
        <div key={index} className="flex gap-2 flex-col">
          <div className="mt-4">
            <label className="block font-medium">
              {items.part}</label>
