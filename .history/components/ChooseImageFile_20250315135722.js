...
export default function ChooseImageFile() {
  ...
  return (
    <div className="">
      ...
      >
        <div className="flex justify-center items-center gap-8 flex-col border-dashed border-slate-600 mt-2 rounded-md border-2 p-8">
  ...
  {files.length > 0 && (
          <div className="grid grid-cols-3 gap-2 mt-4 mb-2 h-full">
          {files.map((fileData, index) => (
            <div
              key={index}
              className="rounded-md w-full bg-slate-500 relative group"
            >
              {fileData.file.type.startsWith("image/") ? (
                <img
                  src={fileData.preview}
                  alt="Preview"
                  className="rounded-md object-cover w-full h-full transition-opacity duration-300 group-hover:opacity-25"
                />
              ) : (
