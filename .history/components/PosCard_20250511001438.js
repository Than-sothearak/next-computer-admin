  ...
  return (
    <>
      <div className="w-full p-4 bg-primary mt-4 rounded-lg">
        <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center  gap-2 text-primary z-20 sm:hidden p-2  bg-red-500 fixed w-22 top-1 ${isOpen ? ' rounded-r-xl left-0' : 'right-0 rounded-full h-full'}`}>
          <MdMenuOpen />
          <IoCartOutline />
        </button>
