  ...
  return (
    <>
      <div className="w-full p-4 bg-primary mt-4 rounded-lg">
        <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center  gap-2 text-primary z-50 sm:hidden p-2  bg-red-500 fixed w-22 top-3 ${isOpen ? ' rounded-r-xl left-0' : 'right-0 rounded-l-xl'}`}>
          <MdMenuOpen />
          Order list
        </button>
      ...
      <div className={`bg-primary mt-4 h-screen p-4
         left-0 z-40 transform rounded-lg top-4  max-sm:fixed transition-transform w-1/2  duration-700 ease-in-out
        ${
          isOpen ? "-translate-x-0 mt-0 w-full " : "translate-x-full w-full"
        }  sm:translate-x-0`}>
        <h1 className="text-xl font-bold mt-10 text-primarytext">Order list</h1>
