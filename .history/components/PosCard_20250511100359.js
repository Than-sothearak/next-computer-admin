...
const PosCard = ({ products }) => {
  ...
  if (storedCart) {
    try {
      ...
      localStorage.removeItem("cartItems"); // Optional: clear the corrupted data
    }
  }
}, []);
  ...
  return (
    <>
      <div className="w-full p-4 bg-primary mt-4 rounded-lg">
        <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center  gap-2 text-primary z-20 sm:hidden p-2  bg-red-500 fixed w-22 top-2 ${isOpen ? ' rounded-r-xl left-0' : 'right-2 rounded-full'} `}>
         
          <IoCartOutline size={24}/>
          <p className="absolute border border-secondary bg-white text-black text-xs right-0 rounded-full w-4 top-0">2</p>
        </button>
        ...
        {isGridView ? <div className="grid grid-cols-2  xl:grid-cols-3 2xl:grid-cols-5 gap-4">
          {products?.map((product, index) => (
            ...
            >
              ...
              <button onClick={() => handleAddToCart(product)} type="button" className="mt-3 bg-blue-600 text-white text-xs py-1 px-2 rounded hover:bg-blue-800">
                Add to Cart
              </button>
            </div>
          ))}
          ...
          {products?.map((product, index) => (
            ...
            >
              ...
              <button onClick={() => handleAddToCart(product)} className="mt-3 bg-blue-600 text-white text-xs py-1 px-2 rounded hover:bg-blue-800">
                Add
              </button>
            </div>
          ))}
