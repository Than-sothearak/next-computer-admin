        ...
        {formData.properties.length > 0 &&
          formData.properties?.slice().reverse().map((part, partIndex) => {
            ...
            return (
              <div
              key={originIndex}
              className={`border border-secondary p-2 rounded-md max-lg:w-full text-sm relative ${!checkIsPropertyEmpty[originIndex] ? 'bg-blue-500 ': ''}`}
            >
                ...
                <div className="w-full ">
                  ...
                  <div className="mt-2">
                    <input
                      ...
                      className={`font-bold w-full p-2 rounded-md  text-xs focus:ring-0 focus:outline-none ${checkIsPropertyEmpty[originIndex] ? 'bg-secondary' : ''}`}
