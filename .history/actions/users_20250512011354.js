...
export async function updateUser(userId, prevState, formData) {
  ...
  try {
    ...
    const isAdmin = role === "admin";
    const isRoleChange = user.isAdmin !== isAdmin;
  ...
  if (isRoleChange) {
      // If role has changed, log out the user
      if (session?.user?._id === userId || session?.user?.isAdmin) {
        return {
          success: true,
          roleChanged: isRoleChange,
          logout: true,  // Signal to log out the user
        };
      }
    }
