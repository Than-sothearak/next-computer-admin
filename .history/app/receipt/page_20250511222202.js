      ...
      <div className="border-b pb-2 mb-2">
        <table className="">
          <thead className="">
            <tr className="w-full flex justify-between border">
              ...
              <th className="w-1/2 border">Qty</th>
              <th className="w-1/3 border">Price</th>
            </tr>
          </thead>
          <tbody>
        ...
        {receipt.items.map((item, idx) => (
          <tr key={idx} className="flex justify-between text-sm w-full">
            <td className="w-full border border">
            
              {item.productName} 
           
            </td>
            <td className="w-1/2 border text-center">{item.quantity}</td>
            <td className="w-1/3 border">${(item.quantity * item.price).toFixed(2)}</td>
          </tr>
        ))}
