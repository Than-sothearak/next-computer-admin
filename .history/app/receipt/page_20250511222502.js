...
export default function ReceiptPage() {
  ...
  return (
    <div className="max-w-lg mx-auto my-10 bg-white p-6 rounded shadow text-sm font-mono">
      ...
      <div className=" pb-2 mb-2">
        <table className="">
          <thead className="">
            <tr className="w-full flex justify-between">
              ...
              <th className="w-1/3 border-b">Price</th>
            </tr>
          </thead>
          <tbody>
        ...
        {receipt.items.map((item, idx) => (
          <tr key={idx} className="flex justify-between text-sm w-full">
            ...
            <td className="w-1/3 border-b text-center">${(item.quantity * item.price).toFixed(2)}</td>
          </tr>
        ))}
