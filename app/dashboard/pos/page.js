...
const posPage = async ({ searchParams }) => {
  ...
  const fetchedProducts = await getProduct(query || "");
  const products = JSON.parse(JSON.stringify(fetchedProducts));

  return (
    <div className="p-4 bg-primary mt-4 rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl font-bold text-primarytext">Point of Sale</h1>
        <SearchComponent placeHolder="Search product..." linkPage="/dashboard/pos" />
      </div>
        ...
        {products.map((product,index) => (
          ...
          >
            <div className="flex flex-col items-center">
              ...
              />
              <h2 className="text-sm font-semibold truncate w-full max-w-[200px]">{product.productName}</h2>
