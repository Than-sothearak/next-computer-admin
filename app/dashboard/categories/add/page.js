...
import { BiCloset, BiMinusCircle, BiPlusCircle } from "react-icons/bi";
...
export default function AddCategory() {
  ...
  const [parts, setParts] = useState([{ part: "", values: [], newValue: "" }]);
  ...
  const handleValueChange = (index, value) => {
    const updatedParts = [...parts];
    updatedParts[index].newValue = value; // Store new value for the specific part
    setParts(updatedParts);
  };
  ...
  const handleAddValue = (index) => {
    const updatedParts = [...parts];
    const newValue = updatedParts[index].newValue.trim();

    if (newValue !== "") {
      updatedParts[index].values.push(newValue);
      updatedParts[index].newValue = ""; // Clear input for that specific part
      setParts(updatedParts);
    }
  };
          ...
          <div>
            <div className="flex gap-4">
              ...
              <div className="w-full flex flex-col gap-2">
                ...
                <select onChange={handleParentCategoryChange} className="w-full p-2 rounded-md bg-slate-700 border-none border-white text-xs focus:ring-0 focus:outline-none">
                  <option value="">Select a parent category</option>

                  <option value="Iwatch">Iwatch</option>
                  <option value="Computer">Computer</option>
                  <option value="Samrtphone">Smartphone</option>
                </select>
              </div>
            </div>
          </div>
          ...
          underline"
            ...
            {parts && (
             parts.map((part, index) => (
              <div key={index} className="bg-slate-600 justify-between p-4 rounded-md w-1/2 max-lg:w-full my-4 space-y-4 text-sm">
